...
export function scanAssignmentHints(lines: string[], uri: Uri): AssignmentRhsHint[] {
...
function resolveCalleeReturnType(
  ...
  const sameFile = withRt.filter(
    (d) => d.uri.fsPath === assignmentUri.fsPath && d.line <= assignmentLine
  );
  ...
  withRt.sort((a, b) => a.uri.fsPath.localeCompare(b.uri.fsPath) || a.line - b.line);
...
export function inferReceiverRootType(
  ...
  const localCandidates = hints.filter(
    (h) =>
      h.lhs === receiver &&
      h.isLocal &&
      h.uri.fsPath === currentUri.fsPath &&
      h.line < hoverLine
  );
  ...
  const globalsDesc = globalCandidates
    .slice()
    .sort((a, b) => b.uri.fsPath.localeCompare(a.uri.fsPath) || b.line - a.line);
...
export function collectReceiverInferenceContext(
  ...
  const hints = [...foreignAssignmentHints, ...scanAssignmentHints(lines, document.uri)];
  ...
  return { hints, defs, currentUri: document.uri };
}
