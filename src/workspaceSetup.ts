...
/**
 * Merge recommended keys into `.vscode/settings.json` in the first workspace folder.
 */
export async function applyRecommendedWorkspaceSettings(): Promise<void> {
  const folder = vscode.workspace.workspaceFolders?.[0];
  if (!folder) {
    void vscode.window.showWarningMessage("Ren'Py IntelliSense: open a workspace folder first.");
    return;
  }
